(function(){
	...
	function reloaded(){
		...
		Array.from(data).forEach(element => {
				...
				updateCartTotal(price.replace('€',''), true)
		  ...
		  var cartAddBtns = document.getElementsByClassName('js-cd-add-to-cart'),
			...
			$('.cable-choose button').click(function(){
				...
				if (!$this.hasClass('btn-success')) {
					$('.cable-choose button').css({"background-color":"","color": "black"})
					$('.cable-choose button').removeClass('btn-success')
					$this.addClass('btn-success');
					$this.css({"background-color":"#1f1f1f","color": "white","box-shadow":"grey"});
			...
			function addProduct(target) {
				...
				qnt = document.getElementById("quantity").value
				target = '<li class="cd-cart__product"><div class="cd-cart__image"><a href="#0"><img src="'+prodimg+'" alt="placeholder"></a></div><div class="cd-cart__details"><h3 class="truncate"><a href="#0">'+prodname+' - ' + seize +'</a></h3><span class="cd-cart__price">€'+(prodprice.replace('€', '')*qnt).toFixed(2)+'</span><div class="cd-cart__actions"><a href="#0" class="cd-cart__delete-item">Delete</a><div class="cd-cart__quantity"><label for="cd-product-'+ productId +'">Qty:'+qnt+'</label><span class="cd-cart__select"><svg class="icon" viewBox="0 0 12 12"><polyline fill="none" stroke="currentColor" points="2,4 6,8 10,4 "/></svg></span></div></div></div></li>';
			...
			function removeProduct(product) {
				...
				res = def.match(/Qty:(.*?)</i)[1];
					...
					productQuantity = res,
					productTotPrice = Number((product.getElementsByClassName('cd-cart__price')[0].innerText).replace('€', ''));
